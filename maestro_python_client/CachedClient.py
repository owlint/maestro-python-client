...
class CachedClient(Client):
    ...
    def launch_task(
        ...
        task_payload = self.__cache_payload(
            queue, task_payload, timeout * (1 + retries) + executes_in
        )
        return super().launch_task(
            owner, queue, task_payload, retries, timeout, executes_in
        )
    ...
    def complete_task(self, task_id: str, result: str) -> None:
        task = self.task_state(task_id)
        result = self.__cache_payload(task.task_queue, result)
    ...
    def launch_task_list(
        ...
        for i, (owner, queue, payload) in enumerate(tasks):
            tasks[i] = (
                owner,
                queue,
                self.__cache_payload(queue, payload, timeout * (retries + 1)),
            )

        return super().launch_task_list(tasks, retries, timeout)
